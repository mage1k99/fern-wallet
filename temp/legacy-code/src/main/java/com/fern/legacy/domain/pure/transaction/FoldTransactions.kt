...
import com.fern.wallet.domain.pure.util.mapIndexedNel
import com.fern.wallet.domain.pure.util.mapIndexedNelSuspend
import com.fern.wallet.domain.pure.util.nonEmptyListOfZeros
...
object LegacyFoldTransactions {
    ...
    suspend fun <Arg> foldTransactionsSuspend(
        ...
        valueFunctions: NonEmptyList<suspend (com.fern.base.legacy.Transaction, Arg) -> BigDecimal>,
        arg: Arg
    ): NonEmptyList<BigDecimal> = sumTransactionsSuspendInternal(
        transactions = transactions,
        valueFunctions = valueFunctions,
    ...
    internal tailrec suspend fun <A> sumTransactionsSuspendInternal(
        transactions: List<com.fern.base.legacy.Transaction>,
        valueFunctionArgument: A,
        valueFunctions: NonEmptyList<suspend (com.fern.base.legacy.Transaction, A) -> BigDecimal>,
        sum: NonEmptyList<BigDecimal> = nonEmptyListOfZeros(n = valueFunctions.size)
    ): NonEmptyList<BigDecimal> {
        return if (transactions.isEmpty()) {
            sum
    ...
    suspend fun <A> sumTrns(
        transactions: List<com.fern.base.legacy.Transaction>,
        valueFunction: suspend (com.fern.base.legacy.Transaction, A) -> BigDecimal,
        argument: A
    ): BigDecimal {
        return sumTransactionsSuspendInternal(
            transactions = transactions,
